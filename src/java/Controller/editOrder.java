...
public class editOrder extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int status = checkoutDao.updateOrder(ch); 

        if(status>0){    
            request.getRequestDispatcher("updateOrderSuccess.jsp").include(request, response);  
        }else{  
            request.getRequestDispatcher("updateOrderError.jsp").include(request, response);  
        }  
